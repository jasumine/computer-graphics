	...
	const WCHAR *vsFilename, const WCHAR *psFilename)
{
	...
	// Get a count of the elements in the layout. 데이터의 양
	...
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix)
{
	...
	// Lock the constant buffer so it can be written to. cpu->gpu로 정보를 전달하는 역할
	...
	// gpu는 정보를 받을때 어느 메모리 주소에 넣을지 정해주어야한다.
	return true;
}
